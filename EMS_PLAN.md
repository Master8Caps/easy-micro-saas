...
- [x] **Invite setup flow** — invited users redirected to `/setup` page (not dashboard) to set name + password before first use
- [x] `/setup` page: standalone client component with Full Name, Password, Confirm Password fields
- [x] Password set via `supabase.auth.updateUser({ password })` (no current password needed — active session from invite link)
- [x] Profile name saved via existing `updateProfile()` server action
- [x] Invite email: raw Supabase URL hidden — fallback link replaced with "click here to accept your invitation" hyperlink
...
- [x] Migration 00016: rating + engagement columns on content_pieces (rating, engagement_views/likes/comments/shares, engagement_logged_at)
...
- [x] Engagement sub-weights: views ×1, likes ×3, comments ×5, shares ×4
- [x] `computeEngagementRaw()` and `computeCompositeScore()` helpers in score-utils.ts
- [x] `loadPerformanceScores()` updated to use composite scoring (fetches content pieces with engagement/rating alongside links)
- [x] `loadLearningInsights()` — pattern extraction from scored content (top performers, patterns, underperformers, thumbs-down)
- [x] Brain generation enhanced — `buildPerformanceContext()` injected into prompt when insights available
- [x] Content generation enhanced — `buildContentPerformanceContext()` injected into prompt with top-performing examples
- [x] Cold start: no context when zero data; sparse data (<5 pieces) gets directional caveat
- [x] `RatingButtons` component — thumbs up/down on content cards with optimistic updates
- [x] `EngagementPopover` component — log views/likes/comments/shares on posted content with popover form
- [x] Rating buttons on content page, campaign panel, and schedule page (all lifecycle stages)
- [x] Engagement popover on content page, campaign panel, and schedule page (posted pieces only)
...
### Ad Management & Optimization System (2026-03-17)

**Architecture:** AI Marketing Strategist — generates, packages, and optimizes ads across platforms. Manual "copilot" mode (copy-paste deployment packages) with adapter pattern for future direct API integrations.

**Target:** Solo founders first (Phase 1), agencies later (Phase 2).

**Competitive angle:** CRUSH AI automates posting but is Meta-only ($99/mo). We automate the *thinking* — strategy, creative, optimization — across any platform.

#### Database (Migrations 00021–00022)
- [x] `ad_campaigns` table — user_id, product_id, platform (meta/google/linkedin/tiktok), objective, status (draft/active/paused/completed), daily_budget, total_budget, audience_targeting (JSONB)
- [x] `ad_sets` table — groups within campaigns, targeting overrides, optional budget override
- [x] `ads` table — individual creatives linked to content_pieces + existing campaigns (angles), headline/body/CTA/image/video/destination URL
- [x] `ad_performance` table — daily snapshots with generated columns (CPC, CPM, CTR, ROAS via `GENERATED ALWAYS AS STORED`), upsert support via unique index with `NULLS NOT DISTINCT`
- [x] `optimization_recommendations` table — AI-generated advice (budget_shift, creative_refresh, audience_adjust, pause, scale)
- [x] Full RLS policies on all 5 tables (user_id direct or via ad_campaigns join)
- [x] `moddatetime` triggers on ad_campaigns, ad_sets, ads, ad_performance
- [x] Indexes: campaign+date, ad+date, product+date, user+status, recommendations user+status

#### Platform Adapter Pattern
- [x] `PlatformAdapter` interface — generatePackage, formatInstructions, parsePerformance, validateCreative, getCreativeSpecs
- [x] `DeploymentPackage` type — platform-specific copy-paste-ready output with deployment steps
- [x] `CreativeSpecs` type — per-platform character limits, image formats, CTA options, video specs
- [x] `MetaManualAdapter` — Facebook/Instagram (40-char headline, 125-char body, 1080x1080/1200x628/1080x1920 images)
- [x] `GoogleManualAdapter` — Search/Display/PMax (30-char headline, 90-char description)
- [x] `LinkedInManualAdapter` — Sponsored Content (70-char headline, 150-char body, B2B audience formatting)
- [x] `TikTokManualAdapter` — Video-first with warnings (100-char limits, 9:16 priority)
- [x] Adapter registry with `getAdapter(platform)` lookup — `apps/app/lib/adapters/index.ts`

#### Server Actions
- [x] `ad-campaigns.ts` — createAdCampaign, updateAdCampaign, deleteAdCampaign, getAdCampaigns, getAdCampaign (with nested ad_sets + ads)
- [x] `ad-performance.ts` — upsertPerformance (conflict-based upsert), getPerformance (date range support), getProductPerformanceSummary
- [x] `ad-generation.ts` — generateAdPackage: loads product + avatars + top-rated ad copy, generates 3 variations via Claude Sonnet, validates through platform adapter, returns deployment packages
- [x] `ad-optimization.ts` — getCampaignHealthAlerts (Level 1 rule-based: CTR/ROAS/CPC checks), analyzeAndRecommend (Level 2 AI cross-campaign analysis via Claude), getRecommendations, updateRecommendation

#### AI Optimization Engine (3 Levels)
- [x] **Level 1: Campaign Health** — rule-based alerts triggered on performance entry (CTR < 1%, ROAS < 1x, high CPC)
- [x] **Level 2: Cross-Campaign Optimization** — Claude analyzes full performance dataset, generates budget_shift/creative_refresh/audience_adjust/pause/scale recommendations (needs 2+ campaigns)
- [ ] **Level 3: Creative Learning Loop** — deferred to future phase (connects content ratings to ad performance)

#### UI Pages
- [x] "Ads" nav item in sidebar (between Content and Schedule)
- [x] `/ads` — Campaign list with platform/status/product filters, "New Campaign" button
- [x] `/ads/new` — 3-step guided creation: basics (product, name, platform, objective) → budget & destination → AI-generated deployment packages with Copy All + deployment steps
- [x] `/ads/[id]` — Campaign detail with status management, stats grid (spend/clicks/conversions/CTR/ROAS), performance data logging form, performance history table, optimization recommendations with accept/dismiss
- [x] `/ads/optimize` — Optimization dashboard with per-product AI analysis trigger, pending recommendations list
- [x] Dashboard "Ad Campaigns" summary card — active count + latest recommendation

#### Important Notes
- **Migrations require PostgreSQL 15+** for `NULLS NOT DISTINCT` on the ad_performance upsert index. Supabase projects created recently should be fine.
- **No direct API integrations yet** — all adapters are "manual mode" generating copy-paste deployment packages. Architecture supports swapping to API adapters later.
- **Ad generation uses Claude Sonnet** — generates 3 variations per campaign with platform-specific constraints
...
- [ ] Invite email: no raw Supabase URL visible — fallback shows "click here to accept your invitation" hyperlink
- [ ] Invited user clicks "Accept invitation" → lands on `/setup` page (not dashboard)
- [ ] `/setup` page: enter name + password → "Complete setup" → redirects to dashboard
- [ ] Sign out, then sign in with email + new password → works
...
> Migration 00020 — `digest_unsubscribed` column on profiles. Applied.
> Cron: `vercel.json` → `/api/cron/weekly-digest` every Monday 9AM UTC.
> Env vars: `CRON_SECRET` (required — see below).

#### Cron Endpoint
- [ ] Set `CRON_SECRET` env var in Vercel (both preview + production)
- [ ] Trigger manually: `curl -H "Authorization: Bearer $CRON_SECRET" https://app.easymicrosaas.com/api/cron/weekly-digest`
- [ ] Response returns `{ sent, skipped, errors }` JSON
- [ ] Without auth header → 401 Unauthorized
- [ ] With wrong secret → 401 Unauthorized
- [ ] Without `CRON_SECRET` env var set → 500 "CRON_SECRET not configured"

#### Digest Email Content
- [ ] Email arrives with subject "Your Weekly Marketing Digest — Easy Micro SaaS"
- [ ] Header shows total clicks, posts this week, top performer product name
- [ ] Per-product cards show clicks, views, likes, comments, shares, composite score
- [ ] Product names and content titles are properly escaped (no raw HTML injection)
- [ ] Max 5 products shown; if more, "and X more products" link appears
- [ ] Action items section present with actionable suggestions
- [ ] CTA button links to dashboard

#### Unsubscribe
- [ ] Unsubscribe link appears at bottom of digest email
- [ ] Click unsubscribe → "You've been unsubscribed" confirmation page
- [ ] After unsubscribe, next cron run skips that user (no email sent)
- [ ] Tampered/invalid unsubscribe link → "Invalid unsubscribe link" error
- [ ] User with no activity → skipped (no empty digest sent)
...
#### View Switcher
- [ ] Schedule page shows "Week | Month" toggle buttons
- [ ] Default view is "Week" (existing behavior preserved)
- [ ] Click "Month" → calendar switches to full month grid
- [ ] Click "Week" → calendar switches back to week view
- [ ] View preference reflected in URL (`?view=month` or `?view=week`)

#### Month Grid
- [ ] Month grid shows correct days (Mon–Sun columns, proper day count)
- [ ] Today's cell has visual highlight
- [ ] Days with scheduled content show colored status dots (max 3 dots per day)
- [ ] Days with more than 3 pieces show "+N" badge
- [ ] Click a day → side panel opens showing pieces for that day
- [ ] Navigate to previous/next month → URL updates with `?month=YYYY-MM`
- [ ] "Back to this month" link appears when viewing other months
- [ ] Month/year label displays correctly (e.g. "March 2026")

#### Month ↔ Week Integration
- [ ] Switch from month to week → preserves date context (lands on relevant week)
- [ ] Switch from week to month → preserves date context (lands on relevant month)
- [ ] Scheduling a piece in week view → dot appears in month view
- [ ] Unscheduling a piece → dot disappears from month view

### Drag-and-Drop Scheduling

> No new migrations required. Uses `@dnd-kit/core`.

#### Basic Drag
- [ ] Hover over an unscheduled content card → grab cursor appears
- [ ] Click and drag card → drag overlay follows mouse
- [ ] Drop card on a calendar day → piece becomes scheduled for that day at 9:00 AM UTC
- [ ] Card disappears from unscheduled pool and appears on calendar
- [ ] Server action fires (check DB — `scheduled_for` timestamp set)

#### Reschedule & Unschedule
- [ ] Drag a scheduled card from one day to another → piece moves to new day
- [ ] Drop a scheduled card on "Unscheduled" zone → piece returns to unscheduled pool
- [ ] Optimistic update: card moves immediately (before server response)
- [ ] If server action fails → card snaps back to original position

#### Touch Support
- [ ] On mobile/tablet: long-press (200ms) a card → drag begins
- [ ] Small finger movements (< 5px) don't trigger drag (tolerance)
- [ ] Drop works on touch devices

#### Both Views
- [ ] DnD works in week view (drop on day columns)
- [ ] DnD works in month view (drop on day cells)
- [ ] DnD from unscheduled to either view works
...
#### Hamburger Menu
- [ ] Below 768px viewport → top bar with hamburger icon + logo appears
- [ ] Desktop sidebar hidden on mobile
- [ ] Tap hamburger → sidebar slides in from left as overlay
- [ ] Sidebar overlay has backdrop (semi-transparent background)
- [ ] All nav links work from mobile sidebar
...
- [ ] Navigate to new page → sidebar auto-closes

#### Responsive Breakpoints
- [ ] 375px (iPhone SE) → mobile nav, content readable
- [ ] 768px (iPad) → transition point, mobile nav still active
- [ ] 769px+ → desktop sidebar returns, mobile nav hidden
- [ ] Resize browser across breakpoint → switches correctly

### Ad Management & Optimization (from 2026-03-17)

> Migrations 00021–00022 — **Must be applied before testing.**

#### Prerequisites
- [ ] Run `supabase db push` or apply migrations 00021 + 00022 via Supabase dashboard
- [ ] Confirm tables exist: ad_campaigns, ad_sets, ads, ad_performance, optimization_recommendations
- [ ] Confirm generated columns work: insert a test ad_performance row and verify cpc/cpm/ctr/roas auto-calculate

#### Navigation
- [ ] "Ads" appears in sidebar between Content and Schedule
- [ ] "Ads" link navigates to `/ads`
- [ ] Active state highlights correctly when on `/ads`, `/ads/new`, `/ads/[id]`, `/ads/optimize`

#### Ads List Page (`/ads`)
- [ ] Empty state shows "No ad campaigns yet" with link to create first
- [ ] Platform filter works (Meta/Google/LinkedIn/TikTok)
- [ ] Status filter works (Draft/Active/Paused/Completed)
- [ ] Product filter works (shows user's products)
- [ ] "New Campaign" button navigates to `/ads/new`
- [ ] Campaign cards show name, status badge, platform, objective, budget, ad count
- [ ] Clicking a campaign navigates to `/ads/[id]`

#### New Campaign Flow (`/ads/new`)
- [ ] Step 1: Product dropdown populated, campaign name input, platform selection (4 buttons), objective selection (5 options)
- [ ] Step 1: "Next" button disabled until name is entered
- [ ] Step 2: Daily budget, total budget (optional), destination URL, additional context (optional)
- [ ] Step 2: "Back" returns to step 1 with data preserved
- [ ] Step 2: "Generate Ad Packages" calls AI and shows loading state
- [ ] Step 3: 3 ad variations displayed with headline, body, CTA, budget, audience, deployment steps
- [ ] Step 3: "Copy All" copies headline + body + CTA to clipboard
- [ ] Step 3: "Done — Go to Campaigns" navigates to `/ads`
- [ ] Step 3: "Regenerate" returns to step 2
- [ ] Error states display correctly (missing product, AI failure, etc.)
- [ ] Campaign is created in database with correct platform, objective, budget

#### Campaign Detail Page (`/ads/[id]`)
- [ ] Back link returns to `/ads`
- [ ] Campaign name, platform, objective, budget displayed in header
- [ ] Status actions shown (Activate for draft, Pause/Complete for active, Resume/Complete for paused)
- [ ] Status changes work and page refreshes
- [ ] Stats grid shows Total Spend, Clicks, Conversions, CTR, ROAS (when performance data exists)
- [ ] "Log Performance" button toggles the entry form
- [ ] Performance form: date picker, spend, impressions, clicks, conversions, conversion value
- [ ] Saving performance data works and appears in the table below
- [ ] Upsert: entering data for an existing date updates instead of duplicating
- [ ] Performance table shows date, spend, impressions, clicks, CTR, conversions, ROAS
- [ ] Recommendations section shows pending recommendations with accept/dismiss buttons
- [ ] Accept/dismiss updates recommendation status

#### Optimization Dashboard (`/ads/optimize`)
- [ ] Product dropdown populated
- [ ] "Analyze Campaigns" disabled when product has < 2 active campaigns
- [ ] "Analyze Campaigns" triggers AI analysis and shows loading state
- [ ] Recommendations appear after analysis with type badge, summary, accept/dismiss
- [ ] Error message shown when insufficient data

#### Dashboard Integration
- [ ] "Ad Campaigns" card appears on main dashboard
- [ ] Shows count of active campaigns
- [ ] Shows latest recommendation summary (if any)
- [ ] Card links to `/ads`

#### Edge Cases
- [ ] Creating a campaign with no products in the system → shows empty product dropdown
- [ ] Navigating to `/ads/[invalid-id]` → 404 page
...
4. **Meta Ad Library API integration** — ⚠️ **DEFERRED** (see research notes below)
5. ~~**Weekly performance summary / digest** — scheduled email or in-app summary of campaign performance~~ **Done** — Vercel Cron (Monday 9AM UTC) + Resend + HMAC-signed unsubscribe
6. ~~**Ad management & optimization system** — AI-powered ad package generation, performance tracking, campaign health checks, cross-campaign optimization~~ **Done** — platform adapter pattern (Meta/Google/LinkedIn/TikTok), manual deployment packages, 3-tier optimization engine
7. **Assisted posting workflow** — copy + reminders to help users actually post their content (low priority — deferred)

> **Meta Ad Library Research (2026-03-13):**
> The official Meta Ad Library API only gives programmatic access to political/social issue ads — **commercial ads are NOT available via API**. The web UI still shows commercial ads but there's no API for it. Options:
> - **AdLibrary.com** ($29 first month, $179/mo) — third-party API, covers Meta + Google + TikTok + more, reliable
> - **Apify scrapers** (~$3-6 per 1,000 ads) — scrapes the public web UI, fragile (breaks on Meta UI updates), ToS risk
> - **Build own scraper** — dev time only, very fragile, Meta actively fights scrapers
> - **Pivot idea:** Skip external ads entirely, build a "swipe file" system where AI generates ad variations from proven frameworks (PAS, AIDA, BAB) with industry examples
>
> **Decision:** Deferred until we decide which approach (or whether to skip entirely). Revisit after social integrations and Stripe are done.

**Self-learning v2 (later):**
- Metricool / platform API integrations for auto-pulling metrics
- AI-assisted content editing with prompts (edit via natural language instruction)
- Deeper pattern analysis with pgvector embeddings

### Short Term
- [ ] ~~Meta Ad Library API integration~~ — **DEFERRED** (see research notes in "Immediate" section)
- [ ] ~~Direct social media integrations~~ — **DEFERRED** (see research notes below)
- [x] ~~Weekly performance summary / digest~~ **Done** — weekly digest email via Vercel Cron + Resend (3-ingredient email: dashboard header, product cards, action items)
- [x] ~~Schedule page: month view option~~ **Done** — traditional calendar grid with view switcher, YYYY-MM URL params, status dots
- [ ] Assisted posting workflow — copy + reminders to help users actually post their content (low priority)
- [ ] pgvector embeddings for semantic content similarity

> **Social Media API Integration Research (2026-03-13):**
> Costs and approval requirements per platform:
> - **Facebook + Instagram** (Meta Graph API) — **Free**, App Review takes 2-7 days, one integration = two platforms. Full publishing (text, images, videos, carousels, Reels, Stories) + native scheduling + full engagement data.
> - **X / Twitter** — **$200/mo minimum** (Basic tier). Free tier is useless (write-only, 500 posts/mo). Pro = $5K/mo. New pay-as-you-go pilot in closed beta.
> - **LinkedIn** — **Free** but requires LinkedIn Marketing Partner approval (weeks-months). Must be registered business + screen recording demo. Hard to get approved.
> - **TikTok** — **Free** but unaudited apps are nearly unusable (posts default to private, 5 users/day). Audit takes 1-4 weeks.
>
> **Concern:** X at $200/mo is a recurring cost per our app, not per user. LinkedIn approval is slow/uncertain. Adds operational cost before we have paying users.
>
> **Alternative considered:** Metricool or similar scheduling tool as middleware — but unclear how that works for multi-tenant SaaS (each user would need their own Metricool account or we'd need their API).
>
> **Decision:** Deferred. Revisit when we have paying users to justify the X API cost. Consider starting with Meta (free) as first integration. Explore whether Metricool has a partner/reseller API that could simplify multi-platform posting.
...
- [x] ~~Mobile responsive navigation~~ **Done** — hamburger menu below 768px, sidebar slide-in overlay with backdrop, Escape dismiss, body scroll lock, auto-close on route change
- [x] ~~Schedule page: drag-and-drop scheduling~~ **Done** — @dnd-kit/core with PointerSensor + TouchSensor, optimistic updates, works in both week and month views, bidirectional (schedule/reschedule/unschedule)
- [ ] Mobile responsive improvements (complex pages: schedule grid, analytics charts)
...
| 2026-03-17 | Manual-first ad management (no platform APIs) | Avoids expensive/slow platform verification processes; copy-paste deployment packages still save users hours of creative/strategy work; adapter pattern allows API swap-in later |
| 2026-03-17 | Platform adapter pattern for ads | Each platform (Meta/Google/LinkedIn/TikTok) gets an adapter implementing a shared interface; Day 1 = manual adapters, future = API adapters; rest of system doesn't change |
| 2026-03-17 | Campaign structure mirrors Meta/Google | ad_campaigns → ad_sets → ads hierarchy matches how ad platforms actually work; makes future API integration a natural fit |
| 2026-03-17 | Postgres generated columns for ad metrics | CPC, CPM, CTR, ROAS computed as `GENERATED ALWAYS AS STORED` — always consistent, zero application code, immutable |
| 2026-03-17 | Denormalized user_id on ad_sets and ads | Avoids deep join chains for RLS policies; common Supabase pattern for performance |
| 2026-03-17 | `ad_campaigns` separate from `campaigns` | `campaigns` = marketing angles/hooks per avatar (existing). `ad_campaigns` = paid ad campaigns on platforms (new). `ads.campaign_id` FK connects an ad creative back to its marketing angle |
